...
+(void) getImagePathFromImagePicker:(UIViewController *)viewController withCompletionHandler:(void (^)(UIImage *,NSString* ))callBack
{
	ImagePicker* picker = [ImagePicker getImagePickerInstance];
	
	picker.delegate = picker;
	picker.sendImagePath = callBack;
...
-(void)imagePickerControllerDidCancel:(UIImagePickerController *)picker
{
	...
	if(self.sendImagePath)
		self.sendImagePath(nil,nil);
}
...
- (void)imagePickerController:(UIImagePickerController *)picker didFinishPickingMediaWithInfo:(NSDictionary *)info
{
	if(self.sendImagePath)
	{
		...
		self.sendImagePath(info[UIImagePickerControllerOriginalImage],[self saveImageToDirectory:image]);
